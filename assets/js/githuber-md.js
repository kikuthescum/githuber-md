...
(function ($) {
  $(function () {
    ...
    spellcheck_dictionary_dir =
      "https://spellcheck-dictionaries.github.io/" + spellcheck_lang + "/";

    is_support_inline_keyboard_style =
      config.support_inline_code_keyboard_style === "yes";
    ...
    global_editormd_config = {
      ...
      matchWordHighlight:
        config.editor_match_highlighter === "yes" ? "onselected" : false,
      ...
      toolbarHandlers: {
        ...
        "align-left": function (cm, icon, cursor, selection) {
          cm.replaceSelection(
            "<div class='align-left'>" + selection + "</div>"
          );
          if (selection === "") {
            cm.setCursor(cursor.line, cursor.ch + 24);
          }
        },
        "align-center": function (cm, icon, cursor, selection) {
          cm.replaceSelection(
            "<div class='align-center'>" + selection + "</div>"
          );
          if (selection === "") {
            cm.setCursor(cursor.line, cursor.ch + 26);
          }
        },
        "align-right": function (cm, icon, cursor, selection) {
          cm.replaceSelection(
            "<div class='align-right'>" + selection + "</div>"
          );
          if (selection === "") {
            cm.setCursor(cursor.line, cursor.ch + 25);
          }
        },
      },
      lang: {
    ...
    wp.media.editor.insert = function (html_str) {
      var content = githuber_md_editor.getMarkdown();
      var cursorPos = githuber_md_editor.getCursor();
      console.log(content);
      ...
      beforeCursor += lines[cursorPos.line].substring(0, cursorPos.ch);
      afterCursor = lines[cursorPos.line].substring(cursorPos.ch) + "\n";

      for (let i = cursorPos.line + 1; i < lines.length; i++) {
        afterCursor += lines[i] + "\n";
      }

      if (beforeCursor.includes("<div") && afterCursor.includes("</div>")) {
        console.log("Skip Processing");
        githuber_md_editor.replaceSelection(html_str);
      } else {
        ...
        if (html_str.substring(0, 4) === "<img") {
          ...
          html_str.substring(0, 7) === "<a href" &&
          -1 !== html_str.indexOf("<img")
        ) {
          ...
          if (image_insert_type === "html") {
            ...
            new_content +=
              "[![" + img_alt + "](" + img_src + ")](" + a_href + ")";
          }
          githuber_md_editor.replaceSelection(new_content);
          image_insert_type = "markdown";
        } else if (
          html_str.substring(0, 1) === "[" &&
          html_str.slice(-1) === "]"
        ) {
          new_content += html_str;
          githuber_md_editor.replaceSelection(new_content);
